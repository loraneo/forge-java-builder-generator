...
public class JavaBuilderGenerator extends AbstractUICommand {
	...
	public UICommandMetadata getMetadata(UIContext context) {
		return Metadata.forCommand(JavaBuilderGenerator.class).name("Generate builder").category(
				JavaUtilsCategory.instance());
	}
	...
	private Optional<JavaResource> getJavResource(Object resource) {
		return Optional.of(resource).filter(p -> JavaResource.class.isAssignableFrom(p.getClass())).map(
				p -> (JavaResource) p);
	}
	...
	private JavaClassSource addBuilder(JavaClassSource resource) {
		...
		resource.getFields()
			.stream()
			.forEach(p -> this.createGetter(resource, p));
		...
		resource.getFields()
			.stream()
			.forEach(p -> this.createBuildMethod(resource, neastedType, p.getName(), p.getType().getQualifiedNameWithGenerics()));
	...
	private void createBuildMethod(JavaClassSource original, JavaClassSource target, String fieldNane, String type) {
		target.addMethod()
				.setPublic()
				.setStatic(false)
				.setName(fieldNane)
